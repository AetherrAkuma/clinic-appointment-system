<?php
session_start();
require_once '../includes/db_connection.php'; // Go up one level from 'assistants/'
require_once '../includes/config.php';     // Go up one level from 'assistants/'

// --- Security Check ---
// Redirects to login if user is not logged in or is not an assistant.
...
$doctor_assigned_id = null; // To store the ID of the doctor this assistant is linked to
$doctor_name = "N/A";

// Fetch the doctor ID linked to this assistant
$stmt = $conn->prepare("SELECT doctor_id FROM assistants WHERE assistant_id = ?");
...
if ($result->num_rows > 0) {
    ...
    if ($doctor_assigned_id) {
        $stmt_doctor = $conn->prepare("SELECT first_name, last_name FROM doctors WHERE doctor_id = ?");
        $stmt_doctor->bind_param("i", $doctor_assigned_id);
        $stmt_doctor->execute();
...
if ($doctor_assigned_id) {
    ...
    $stmt_appointments = $conn->prepare("
        ...
        WHERE a.doctor_id = ? AND a.status = 'Scheduled' AND CONCAT(a.appointment_date, ' ', a.appointment_time) >= ?
        ORDER BY a.appointment_date ASC, a.appointment_time ASC
        LIMIT 5
    ");
    $stmt_appointments->bind_param("is", $doctor_assigned_id, $current_datetime);
...
<head>
    ...
    <title>Assistant Dashboard - Clinic Appointment System</title>
    <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.1/css/all.min.css" rel="stylesheet">
    <style>
        body { font-family: 'Segoe UI', sans-serif; background-color: #f0f2f5; }
        .navbar { background-color: #5d6d7e; /* Steel blue */ padding: 10px 20px; }
        ...
        .sidebar {
            ...
            background-color: #7f8c8d; /* Wet Asphalt */
            padding-top: 66px; color: #ecf0f1; box-shadow: 2px 0 5px rgba(0,0,0,0.1);
        }
        ...
        .sidebar .list-group-item:hover {
            background-color: #5d6d7e; /* Darker steel blue */
            border-left-color: #27ae60; /* Emerald green on hover */
            color: #fff;
        }
        .sidebar .list-group-item.active {
            background-color: #27ae60; /* Emerald green for active link */
        ...
        .card-custom h4 { color: #34495e; margin-bottom: 15px; }
        .appointment-list-card .table thead th { background-color: #f8f9fa; color: #34495e; border-bottom: 2px solid #dee2e6; }
        .appointment-list-card .table tbody tr:hover { background-color: #f2f2f2; }
        .no-appointments {
            text-align: center;
            color: #7f8c8d;
            padding: 20px;
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark fixed-top">
        ...
        <div class="collapse navbar-collapse">
            <ul class="navbar-nav ml-auto">
                <li class="nav-item">
                    <span class="nav-link">Welcome, **<?php echo htmlspecialchars($assistant_username); ?>**</span>
                </li>
                <li class="nav-item">
    ...
    <div class="sidebar">
        <div class="list-group list-group-flush">
            <a href="<?php echo BASE_URL; ?>assistants/dashboard.php" class="list-group-item list-group-item-action active">
    ...
    <div class="main-content">
        <div class="container-fluid">
            <h1 class="mb-4 text-dark">Assistant Dashboard</h1>

            <div class="card-custom mb-4">
                <h4>Welcome, <?php echo htmlspecialchars($assistant_username); ?>!</h4>
                <p class="text-secondary mb-0">You are assisting **Dr. <?php echo htmlspecialchars($doctor_name); ?>**.</p>
                <p class="text-secondary mb-0">Use the sidebar to manage appointments, doctor's schedule, and patient records.</p>
            </div>

            <div class="card-custom appointment-list-card">
                <h4><i class="fas fa-calendar-check mr-2"></i>Upcoming Appointments for Dr. <?php echo htmlspecialchars($doctor_name); ?> (Next 5)</h4>
                ...
                <?php else: ?>
                    <p class="no-appointments">No upcoming appointments found for Dr. <?php echo htmlspecialchars($doctor_name); ?>.</p>
                <?php endif; ?>
            </div>
